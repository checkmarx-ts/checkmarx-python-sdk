...
from .dto import CxXMLResults, Line, Path, PathNode, Query, Result, Snippet
        ...
        for result_tag in result_tags:
            ...
            for path_node_tag in path_node_tags:
                ...
                if len(path_node_tag) >= 8:
                    ...
                    line = Line(number=int(inner_line_number), code=code)
            ...
            path = Path.from_dict(item=path_tag.attrib, path_nodes=path_nodes)
            result = Result.from_dict(item=result_tag.attrib, path=path)
            results.append(result)
        query = Query.from_dict(item=query_tag.attrib, results=results)
        queries.append(query)
    return CxXMLResults.from_dict(root.attrib, queries=queries)
