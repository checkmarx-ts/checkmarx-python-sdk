...
class ScansODataAPI(object):
    ...
    def get_all_data_for_a_specific_scan_id(self, scan_id: int) -> dict:
        ...
        """
        url = f"{self.base_url}/Cxwebinterface/odata/v1/Scans({scan_id})"
    ...
    def get_number_of_loc_scanned_for_a_specific_scan(self, scan_id: int) -> int:
        ...
        Returns:
            number_of_loc (int)
        """
        url = f"{self.base_url}/Cxwebinterface/odata/v1/Scans({scan_id})?$select=LOC"
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")[0].get("LOC")
    ...
    def get_number_of_loc_scanned_for_all_scan(self) -> List[dict]:
        ...
        """
        url = f"{self.base_url}/Cxwebinterface/odata/v1/Scans?$select=LOC,Id"
        response = self.api_client.call_api(method="GET", url=url)
    ...
    def get_last_scan_id_of_a_project(self, project_id: int) -> int:
        ...
        """
        url = (
            f"{self.base_url}/Cxwebinterface/odata/v1/Projects({project_id})"
            f"//Scans?$orderby=Id%20desc&$top=1&$select=Id"
        )
        response = self.api_client.call_api(method="GET", url=url)
    ...
    def get_last_scan_of_a_project(self, project_id: int) -> dict:
        ...
        url = (
            f"{self.base_url}/Cxwebinterface/odata/v1/Projects({project_id})"
            f"/Scans?$orderby=Id%20desc&$top=1"
        )
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")[0]
    ...
    def get_last_full_scan_id_of_a_project(self, project_id: int) -> int:
        ...
        """
        url = (
            f"{self.base_url}/Cxwebinterface/odata/v1/Projects({project_id})/Scans"
            f"?$filter=IsIncremental%20eq%20false&$orderby=Id%20desc&$top=1&select=Id"
        )
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")[0].get("Id")
    ...
    def get_last_full_scan_of_a_project(self, project_id: int) -> dict:
        ...
        url = (
            f"{self.base_url}/Cxwebinterface/odata/v1/Projects({project_id})/Scans"
            f"?$filter=IsIncremental%20eq%20false&$orderby=Id%20desc&$top=1"
        )
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")[0]
    ...
    def get_all_scans_within_a_predefined_time_range_and_their_h_m_l_values_for_a_project(
        ...
        url = (
            ...
            f"&$select=Id,ScanRequestedOn,High,Medium,Low&$orderby=ScanRequestedOn%20desc"
        )
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")
    ...
    def get_the_state_of_each_scan_result_since_a_specific_date_for_a_project(
        ...
        """
        url = (
            f"{self.base_url}/Cxwebinterface/odata/v1/Scans?"
            f"$filter=ProjectId%20eq%20{project_id}%20and"
            f"%20ScanRequestedOn%20gt%20{start_date}"
            f"&$expand=Results($expand=State;$select=Id,ScanId,StateId)"
        )
        response = self.api_client.call_api(method="GET", url=url)
        return response.json().get("value")
    ...
    def get_all_scan_id_of_a_project(self, project_id: int) -> List[int]:
        ...
        response = self.api_client.call_api(method="GET", url=url)
        return [item.get("Id") for item in response.json().get("value")]
...
def get_number_of_loc_scanned_for_a_specific_scan(scan_id: int) -> int:
    return ScansODataAPI().get_number_of_loc_scanned_for_a_specific_scan(
        scan_id=scan_id
    )
