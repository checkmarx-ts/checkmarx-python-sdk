...
def get_project_id_name_and_scan_id_list():
    ...
    for project_id_name in project_id_name_list:
        ...
        project_id_name.update({"ScanIdList": list(scan_id_list)})
...
def scan_results_group_by_query_id(original_results):
    ...
    for _, query_id_group in groupby(original_results, lambda r: r.get("QueryId")):
        ...
        false_positive_number = len(
            list(
                filter(
                    lambda r: r.get("ResultState")
                    in ["Not Exploitable", "Proposed Not Exploitable"],
...
def get_all_results_with_count_for_each_project_json_format(
    filter_false_positive=False, threshold=0
):
    """
...
def merge_results_by_similarity_id(first_result_list, second_result_list):
    ...
    Returns:
        list of dict
    """
    similarity_id_list_of_first = [
        item.get("SimilarityId") for item in first_result_list
    ]
...
def get_result(project, filter_false_positive=False, threshold=0):
    ...
    is_for_all_raw_results = (
        True if filter_false_positive is False and threshold == 0 else False
    )
    ...
    if is_for_all_raw_results:
        ...
        if threshold > 1:
            result_list = list(
                filter(lambda r: r.get("TotalNumber") >= threshold, result_list)
            )
...
def get_results_and_write_to_csv_file(
    file_path, field_names, filter_false_positive=False, threshold=0
):
    """
    Args:
        file_path (str):
...
def dump_last_scan_results_statistics_of_each_project_into_csv_file(
    file_path, threshold=0
):
    filed_names = [
        "TeamName",
        "TeamId",
