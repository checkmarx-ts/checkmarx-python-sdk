...
class PolicyEnforcerConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "PolicyEnforcerConfig":
        snake_data: Dict[str, Any] = {underscore(k): v for k, v in data.items()}

        if (
            "enforcement_mode" in snake_data
            and snake_data["enforcement_mode"] is not None
        ):
            snake_data["enforcement_mode"] = EnforcementMode.from_dict(
                snake_data["enforcement_mode"]
            )
        if "path_cache" in snake_data and snake_data["path_cache"] is not None:
            snake_data["path_cache"] = PathCacheConfig.from_dict(
                snake_data["path_cache"]
            )
