...
class ClientProfilesRepresentation:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ClientProfilesRepresentation":
        snake_data: Dict[str, Any] = {underscore(k): v for k, v in data.items()}

        if "profiles" in snake_data and snake_data["profiles"] is not None:
            snake_data["profiles"] = [
                ClientProfileRepresentation.from_dict(item)
                for item in snake_data["profiles"]
            ]
        if (
            "global_profiles" in snake_data
            and snake_data["global_profiles"] is not None
        ):
            snake_data["global_profiles"] = [
                ClientProfileRepresentation.from_dict(item)
                for item in snake_data["global_profiles"]
            ]
        required_fields = []
