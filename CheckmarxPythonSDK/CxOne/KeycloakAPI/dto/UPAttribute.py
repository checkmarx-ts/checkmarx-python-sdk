...
class UPAttribute:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UPAttribute":
        ...
        if "required" in snake_data and snake_data["required"] is not None:
            snake_data["required"] = UPAttributeRequired.from_dict(
                snake_data["required"]
            )
        if "permissions" in snake_data and snake_data["permissions"] is not None:
            snake_data["permissions"] = UPAttributePermissions.from_dict(
                snake_data["permissions"]
            )
        if "selector" in snake_data and snake_data["selector"] is not None:
            snake_data["selector"] = UPAttributeSelector.from_dict(
                snake_data["selector"]
            )
        required_fields = []
