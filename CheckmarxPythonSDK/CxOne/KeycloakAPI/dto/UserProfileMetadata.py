...
class UserProfileMetadata:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "UserProfileMetadata":
        snake_data: Dict[str, Any] = {underscore(k): v for k, v in data.items()}

        if "attributes" in snake_data and snake_data["attributes"] is not None:
            snake_data["attributes"] = [
                UserProfileAttributeMetadata.from_dict(item)
                for item in snake_data["attributes"]
            ]
        if "groups" in snake_data and snake_data["groups"] is not None:
            snake_data["groups"] = [
                UserProfileAttributeGroupMetadata.from_dict(item)
                for item in snake_data["groups"]
            ]
        required_fields = []
