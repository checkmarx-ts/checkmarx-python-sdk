...
class AuthenticationFlowRepresentation:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "AuthenticationFlowRepresentation":
        snake_data: Dict[str, Any] = {underscore(k): v for k, v in data.items()}

        if (
            "authentication_executions" in snake_data
            and snake_data["authentication_executions"] is not None
        ):
            snake_data["authentication_executions"] = [
                AuthenticationExecutionExportRepresentation.from_dict(item)
                for item in snake_data["authentication_executions"]
            ]
        required_fields = []
