...
class ResourceServerRepresentation:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ResourceServerRepresentation":
        snake_data: Dict[str, Any] = {underscore(k): v for k, v in data.items()}

        if (
            "policy_enforcement_mode" in snake_data
            and snake_data["policy_enforcement_mode"] is not None
        ):
            snake_data["policy_enforcement_mode"] = PolicyEnforcementMode.from_dict(
                snake_data["policy_enforcement_mode"]
            )
        if (
            "decision_strategy" in snake_data
            and snake_data["decision_strategy"] is not None
        ):
            snake_data["decision_strategy"] = DecisionStrategy.from_dict(
                snake_data["decision_strategy"]
            )
        if "resources" in snake_data and snake_data["resources"] is not None:
            snake_data["resources"] = [
                ResourceRepresentation.from_dict(item)
                for item in snake_data["resources"]
            ]
