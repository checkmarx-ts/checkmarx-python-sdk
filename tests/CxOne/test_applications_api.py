...
from CheckmarxPythonSDK.CxOne.dto import ApplicationInput, RuleInput
...
def test_create_an_application():
    ...
    application_input = ApplicationInput(
        ...
        rules=[RuleInput(type="project.name.contains", value="test")],
...
def test_get_a_list_of_applications():
    ...
    assert len(app_collection.applications) >= 1
...
def test_update_an_application():
    ...
    is_successful = update_an_application(
...
def test_create_an_application_rule():
    application_id = get_application_id_by_name(name=application_name)
    rule_input = RuleInput(type="project.name.contains", value="happy")
...
def test_get_a_list_of_rules_for_a_specific_application():
    application_id = get_application_id_by_name(name=application_name)
    application_rules = get_a_list_of_rules_for_a_specific_application(
...
def test_get_an_application_rule():
    ...
    if application_rules:
        ...
        application_rule = get_an_application_rule(
            application_id=application_id, rule_id=rule_id
        )
        assert application_rule is not None
...
def test_update_an_application_rule():
    ...
    if application_rules:
        rule_id = application_rules[-1].id
        rule_input = RuleInput(type="project.name.contains", value="happy1")
        is_successful = update_an_application_rule(
...
def test_delete_an_application_rule():
    ...
    if application_rules:
        ...
        is_successful = delete_an_application_rule(
            application_id=application_id, rule_id=rule_id
        )
        assert is_successful is True
