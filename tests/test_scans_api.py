...
def test_update_queued_scan_status_by_scan_id():
    ...
    scan = scan_api.create_new_scan(project_id, is_incremental=False, is_public=True, force_scan=True,
                                    comment="scan from REST API")
    time.sleep(5)
    result = scan_api.update_queued_scan_status_by_scan_id(scan_id=scan.id, status_id=1, status_value="Canceled")
    assert result is True
...
def test_publish_last_scan_results_to_management_and_orchestration_by_project_id():
    project_id = get_project_id()
    scan_api = ScansAPI()
    policy_finding_response = scan_api.publish_last_scan_results_to_management_and_orchestration_by_project_id(project_id)
...
def test_get_the_publish_last_scan_results_to_management_and_orchestration_status():
    project_id = get_project_id()
    scan_api = ScansAPI()
    policy_finding_status = scan_api.get_the_publish_last_scan_results_to_management_and_orchestration_status(project_id)
