...
def test_update_project_by_id():
    ...
    if project_id and team_id:
        project = projects_api.get_project_details_by_id(project_id)
        result = projects_api.update_project_by_id(project_id, project_name=project.name, team_id=team_id)
...
def test_set_project_exclude_settings_by_project_id():
    ...
    if project_id is None:
        pytest.skip("Project 'test1' does not exist")
...
@pytest.mark.skip(reason="Requires a local SSH private key file not available in CI")
def test_set_remote_source_setting_to_git_with_ssh_key():
    projects_api = ProjectsAPI()
    project_name = "test1"
    project_id = projects_api.create_project_if_not_exists_by_project_name_and_team_full_name(
        project_name, team_full_name
    ...
    try:
        result = projects_api.set_remote_source_setting_to_git(
            project_id, url, branch, authentication="ssh", private_key=private_key_content
        )
        assert result is True
    except CxError:
        pass
...
def test_set_remote_source_setting_to_git_using_ssh():
    ...
    branch = "refs/heads/master"
    private_key_file_path = "~/.ssh/id_ed25519"
...
def test_set_project_next_scheduled_scan_to_be_excluded_from_no_code_change_detection():
    project_id = get_project_id()
    projects_api = ProjectsAPI()
    is_successful = projects_api.set_project_next_scheduled_scan_to_be_excluded_from_no_code_change_detection(
        project_id
    )
    assert is_successful is True
...
def test_precheck_team():
    team_id = TeamAPI().get_team_id_by_team_full_name(team_full_name)
    projects_api = ProjectsAPI()
    result = projects_api.precheck_team(team_id)
...
def test_force_scan_on_no_code_changes():
    project_id = get_project_id()
    result = ProjectsAPI().force_scan_on_no_code_changes(project_id=project_id)
    assert result is True


def test_get_path_filter():
    project_id = get_project_id()
    result = ProjectsAPI().get_path_filter(project_id=project_id)
    assert result is not None


def test_set_path_filter():
    project_id = get_project_id()
    result = ProjectsAPI().set_path_filter(project_id=project_id, path_filter="!test,!lib")
    assert result is True


def test_get_project_validity_for_running_incremental_scan():
    project_id = get_project_id()
    result = ProjectsAPI().get_project_validity_for_running_incremental_scan(project_id=project_id)
    assert result is not None
