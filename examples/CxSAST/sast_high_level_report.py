...
class Summary:
    ...
    def __init__(
        self, project_name, fixed, not_exploitable, new, recurrent, custom_fields
    ):
    ...
    def __repr__(self):
        ...
        return (
            f"Summary({self.project_name},{self.fixed},"
    ...
    def __str__(self):
        ...
        return (
            f"Summary[project_name={self.project_name},"
            f"fixed={self.fixed},"
...
def get_all_scans_in_date_range(args):
    ...
    url = (
        f"/Cxwebinterface/odata/v1/Scans?"
...
def get_scan_with_results(scan_id):
    ...
    url = (
        f"/Cxwebinterface/odata/v1/Scans({scan_id})?" "&$expand=Results($expand=State)"
    )
...
def log_result(msg, result):
    ...
    logging.debug(
        f'{msg}: id={result["Id"]},result_id={result["ResultId"]},scan_id={result["ScanId"]},similarity_id={result["SimilarityId"]},path_id={result["PathId"]},query_id={result["QueryId"]}'
    )
...
def get_project_by_name(project_name, team=None):
    ...
    if team:
        return projects_api.get_project_id_by_project_name_and_team_full_name(
            project_name, team.full_name
        )
    else:
...
class Project:
    ...
    def add_scan(self, scan):
        ...
        logging.debug(
            f"scan_requested_on: {scan_requested_on}, self.first_scan_date: {self.first_scan_date}, self.last_scan_date: {self.last_scan_date}"
        )
    ...
    def compare_scans(self):
        ...
        if self.first_scan_id != self.last_scan_id:
            logging.debug(
                f"Comparing scans {self.first_scan_id} and {self.last_scan_id} for project {self.project_name}"
            )
            ...
            return Summary(
                self.project_name,
                len(fixed_similarity_ids),
                count_not_exploitable(last_results),
                len(filter_not_exploitable(new_similarity_ids, last_result_map)),
                len(filter_not_exploitable(recurring_similarity_ids, last_result_map)),
                self.custom_fields,
            )

        else:
            logging.debug(
                f"First and last scan ids are the same ({self.first_scan_id})"
            )
            return None
...
def compare(args):
    ...
    for scan in scans:
        ...
        if project_name not in projects:
            ...
            projects[project_name] = Project(
                project_id, project_name, args.custom_fields
            )
    ...
    header = ["Project Name", "Fixed", "Not Exploitable", "New", "Recurrent"]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--project_name", help="Restrict report to the specified project"
    )
    parser.add_argument(
        "-t", "--team_name", help="Restrict report to the specified team"
    )
    parser.add_argument(
        "start_date", type=valid_date, help="Start date of report period"
    )
    parser.add_argument("end_date", type=valid_date, help="End date of report period")
