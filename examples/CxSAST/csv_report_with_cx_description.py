...
def get_command_line_arguments():
    ...
    parser.add_argument(
        "--cxsast_base_url",
        required=True,
        help="CxSAST base url, for example: https://localhost",
    )
    ...
    parser.add_argument(
        "--project_names", required=True, help="example: project1,project2"
    )
...
def generate_report(scan_id: int, project_name: str, report_file_path: str):
    ...
    logger.info(
        f"finish generate csv report for scan id {scan_id}, report file path: {report_file_path}"
    )
...
def update_csv_report(cx_report_file_path, query_collections, query_description_func):
    ...
    with open(cx_report_file_path, newline="", encoding="utf-8-sig") as csvfile:
        ...
        for row in reader:
            ...
            query_path_list = (
                query_path.split(":")[0].replace("Version", "").strip().split("\\")
            )
            language = query_path_list[0]
            package_type_name = "_".join(query_path_list[1].split(" "))
            package_name = "_".join(query_path_list[2].split(" "))
            query_id = get_query_id_by_language_group_and_query_name(
                query_collections, language, package_type_name, package_name, query_name
            )
...
if __name__ == "__main__":
    ...
    project_names = (
        project_names.split(",") if "," in project_names else [project_names]
    )
    ...
    for project_name in project_names:
        ...
        project_id = None
        filtered_projects = [
            item for item in project_id_names if item.get("ProjectName") == project_name
        ]
        ...
        if project_id is None:
            ...
            continue
        scan_id = ScansAPI.get_last_scan_id_of_a_project(
            project_id,
        ...
        report_file_path = generate_report(
            scan_id=scan_id, project_name=project_name, report_file_path=file_path
        )
        update_csv_report(
            cx_report_file_path=report_file_path,
